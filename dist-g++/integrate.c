     ...
     register tree fndecl;
{
  ...
  int max_insns = 8 * (8 + nargs + 16*TREE_INLINE (fndecl));
  ...
  for (parms = DECL_ARGUMENTS (fndecl); parms; parms = TREE_CHAIN (parms))
	...
	return "function without prototype uses address of parameter cannot be inline";
    }
...
/* Integrate the procedure defined by FNDECL.  Note that this function
   ...
   C++: this function may also cause other inline functions to be
   called (such as when it changes binding contours.  The local
   variable THIS_INLINE_TARGET makes `expand_inline_function' reentrant.
   This can happen because the argument list contains unevaluated terms,
   some of which may be temporaries which have not yet been fixed with
   their DECL_RTL yet.  */
     ...
     rtx structure_value_addr;
{
  ...
  /* C++: watch out!  This could alter `inline_target' by calling an
     inline destructor.  And, any temporary aggregates which live at
     this context level were really created in the containing level,
     and hence may need to live through this call's expansion.  */
