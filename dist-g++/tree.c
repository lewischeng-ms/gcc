     ...
     enum tree_code code;
{
  ...
  switch (type)
    ...
    case 'T':  /* a classtype node */
      length = sizeof (struct tree_classtype);
      /* All data types are put where we can preserve them if nec.  */
      if (obstack != &permanent_obstack)
	obstack = saveable_obstack;
      break;
...
tree
build_classtype_variant (type, offset, vtable)
     tree type;
     tree offset, vtable;
{
  ...
  for (t = m; t; t = CLASSTYPE_NEXT_VARIANT (t))
    if (lo == TREE_INT_CST_LOW (CLASSTYPE_OFFSET (t))
...
/* Return, as a INTEGER_TYPE node, the number of elements for
     ...
     tree basetype, type;
{
  ...
  /* Make a node of the sort we want.  */
  t = make_node (MEMBER_TYPE);

  if (TREE_CODE (type) == FUNCTION_TYPE)
    {
      /* Member functions behave differently then
	 normal functions.  */
      tree argtypes = TYPE_ARG_TYPES (type);

      argtypes = tree_cons (NULL_TREE, build_pointer_type (basetype), argtypes);

      type = build_function_type (TREE_TYPE (type), argtypes);
    }
  else if (TREE_CODE (type) == MEMBER_TYPE)
    {
      warning ("member of member type");
      type = TREE_TYPE (type);
    }

  TREE_TYPE (t) = type;
  TYPE_MEMBER_OF (t) = basetype;

  /* Cache this for easy lookup later.  */
  TYPE_ARG_TYPES (t) = TYPE_ARG_TYPES (type);
