...
%token OPERATOR_PUSH OPERATOR_POP
%token <itype> SCOPE
...
%type <ttype> new object primary_no_id
		...
		do_scoped_member:
		  {
			...
			if (basetype == TYPE_MAIN_VARIANT (TREE_TYPE ($1))
			    || get_base_type (basetype, TYPE_MAIN_VARIANT (TREE_TYPE ($1))))
			  {
			    $$ = build_component_ref (build_scoped_ref ($1, $2), $3, 1);
			  }
			else
			  {
		  ...
		  if (is_aggr_typedef_or_else (basetype))
			  ...
			  || get_base_type (basetype, TYPE_MAIN_VARIANT (TREE_TYPE ($1))))
			{
			  ...
			  if (TREE_CODE ($1) == INDIRECT_REF)
			    {
			      decl = convert_to_nonzero_pointer (TYPE_POINTER_TO (basetype), TREE_OPERAND ($1, 0));
			    }
			  else
			    {
	...
	| forhead.2
	  ...
	  xexpr ')'
		...
		{ push_momentary ();
		  $<itype>7 = lineno; }
	  stmt
		{ emit_note (input_filename, $<itype>7);
		  expand_loop_continue_here ();
		  if ($6) expand_expr_stmt ($6);
		  pop_momentary ();
		  expand_end_loop ();
...
operator_name:
	...
	| OPERATOR_PUSH
		{
		  $$ = build_nt (OP_EXPR, 0, PUSH_EXPR);
		}
	| OPERATOR_POP
		{
		  $$ = build_nt (OP_EXPR, 0, POP_EXPR);
		}
