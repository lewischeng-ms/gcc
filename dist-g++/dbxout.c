     ...
     int full;
{
  ...
  switch (TREE_CODE (type))
	  ...
	  || TYPE_SIZE (type) == 0)
	{
      ...
      if (CLASSTYPE_N_BASECLASSES (type) && write_symbols_plus)
	{
	  putc ('!', asmfile);
	  putc (TREE_VIA_PUBLIC (type) ? '2'
		: '0',
		asmfile);
	  dbxout_type (CLASSTYPE_BASECLASS (type, 1), 0);
	  putc (',', asmfile);
	  CHARS (2);
	}
      CHARS (11);
	  ...
	  if (TREE_VIRTUAL (type))
	    {
	      ...
	      t = type;
	      while (CLASSTYPE_N_BASECLASSES (t) && TREE_VIRTUAL (CLASSTYPE_BASECLASS (t, 1)))
      ...
      if (write_symbols_plus)
	{
	  putc ('@', asmfile);
	  CHARS (1);
	  dbxout_type (TYPE_MEMBER_OF (type), 0);
	  putc (',', asmfile);
	  CHARS (1);
	  dbxout_type (TREE_TYPE (type), 0);
	}
