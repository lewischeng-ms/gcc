     ...
     struct candidate *final_cp;
{
  ...
  /* We find friend functions from the TREE_TYPE field of the list
     of class functions.  Therefore, the only time we find global
     functions is when they are not the same names as any of the
     functions within the class, or when the parameter list
     we have says to look for global functions (by virtue
     of a first argument of non POINTER_TYPE).  */
  if (IDENTIFIER_CLASS_VALUE (fnname))
    {
      tree type = TREE_TYPE (TREE_VALUE (parms));
  ...
  if (TREE_CODE (TREE_VALUE (functions)) == TREE_LIST)
    ...
    {
      /* Class should use list-of-lists data structure.  */
      if (functions == IDENTIFIER_CLASS_VALUE (fnname))
	abort ();
  ...
  if (TREE_CODE (TREE_VALUE (functions)) == TREE_LIST)
      ...
      while (outer)
	{
	  ...
	  /* Do friends, if any.  */
	  inner = TREE_TYPE (TREE_VALUE (outer));
	  while (inner)
	    {
	      /* Here, inner will be a FRIEND_DECL.  */
	      function = TREE_TYPE (inner);
	      compute_conversion_costs (function, parms, cp, parmlength);
	      if (cp->evil == 0)
		{
     ...
     tree arg3;
{
  ...
  if (IS_AGGR_TYPE (type1))
    fields1 = lookup_fnfields (type1, fnname);
     ...
     tree value, name;
{
  if (value == error_mark_node)
	...
	{
	  tree fields = lookup_fnfields (current_class_type, name);
	  if (fields)
	    {
  ...
  if (TREE_NONLOCAL (value))
	...
	{
	  if (yychar == '(')
	    if (! (IS_AGGR_TYPE (TREE_TYPE (value))
		   && TREE_HAS_CALL_OVERLOADED (TREE_TYPE (value)))
     ...
     tree op;
{
  ...
  if (TREE_CODE (op) != TYPE_EXPR)
    return grokopexpr (op, NULL_TREE, 0);
