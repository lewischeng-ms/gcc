...
struct tree_common
{
  ...
  unsigned has_ctor_attr : 1;
  unsigned needs_ctor_attr : 1;
  unsigned has_dtor_attr : 1;
...
/* Nonzero in a FIELD_DECL means it is a bit-field; it may occupy
   ...
   For a VAR_DECL, means that it must be pushed onto the stack, rather
   than reside within the frame.  Used for creating and passing temporary
   aggregate around.  */
...
#define TREE_OPERATOR(NODE) ((NODE)->common.needs_ctor_attr)
...
#define TREE_HAS_CONSTRUCTOR(NODE) ((NODE)->common.has_ctor_attr)
#define TREE_RETURNS_FIRST_ARG(NODE) ((NODE)->common.has_ctor_attr)

/* Nonzero for _TYPE means that the _TYPE defines
   a destructor.  This implicitly means that a constructor
   is also defined.  */
#define TREE_HAS_DESTRUCTOR(NODE) ((NODE)->common.has_dtor_attr)
...
#define TREE_NEEDS_CONSTRUCTOR(NODE) ((NODE)->common.needs_ctor_attr)
...
#define TREE_NEEDS_DESTRUCTOR(NODE) ((NODE)->common.needs_dtor_attr)
...
#define TREE_PRESERVES_THIS(NODE) ((NODE)->common.private_attr) /* overloaded! */

...
#define TYPE_MEMBER_OF(NODE) ((NODE)->type.max)
...
/* Derived type for type `tree_type'.  */
/* See tree.def for documentation of the use of these fields.  */
...
/* Nonzero for _CLASSTYPE means that the _CLASSTYPE either has
   ...
   defined.  */
#define TREE_HAS_ASSIGNMENT(NODE) ((NODE)->classtype.type_flags.has_assignment_attr)
#define TREE_GETS_ASSIGNMENT(NODE) ((NODE)->classtype.type_flags.gets_assignment_attr)

/* Nonzero for _CLASSTYPE means that operator new and delete are defined,
   respectively.  */
#define TREE_GETS_NEW(NODE) ((NODE)->classtype.type_flags.gets_new_attr)
#define TREE_GETS_DELETE(NODE) ((NODE)->classtype.type_flags.gets_delete_attr)
...
#define TREE_HAS_TYPE_CONVERSION(NODE) ((NODE)->classtype.type_flags.has_type_conversion_attr)

/* Nonzero means that this _CLASSTYPE node can convert itself to an
   INTEGER_TYPE.  */
#define TREE_HAS_INT_CONVERSION(NODE) ((NODE)->classtype.type_flags.has_int_conversion_attr)

/* Nonzero means that this _CLASSTYPE node can convert itself to an
   REAL_TYPE.  */
#define TREE_HAS_REAL_CONVERSION(NODE) ((NODE)->classtype.type_flags.has_float_conversion_attr)

/* Nonzero means that this _CLASSTYPE node overloads operator=(X&).  */
#define TREE_HAS_ASSIGN_REF(NODE) ((NODE)->classtype.type_flags.has_assign_ref_attr)
#define TREE_GETS_ASSIGN_REF(NODE) ((NODE)->classtype.type_flags.gets_assign_ref_attr)

/* Nonzero means that this _CLASSTYPE node has an X(X&) constructor.  */
#define TREE_GETS_INIT_REF(NODE) ((NODE)->classtype.type_flags.gets_init_ref_attr)

/* Nonzero means that this _CLASSTYPE node has an X(X ...) constructor.
   Note that there must be other arguments, or this constructor is flaged
   as being erroneous.  */
#define TREE_GETS_INIT_AGGR(NODE) ((NODE)->classtype.type_flags.gets_init_aggr_attr)
...
#define TREE_HAS_METHOD_CALL_OVERLOADED(NODE) ((NODE)->classtype.type_flags.has_method_call_overloaded)
#define TREE_HAS_WRAPPER(NODE) ((NODE)->classtype.type_flags.has_wrapper)
#define TREE_NEEDS_WRAPPER(NODE) ((NODE)->classtype.type_flags.needs_wrapper)

/* Nonzero means that this _CLASSTYPE node overloads operator().  */
#define TREE_HAS_CALL_OVERLOADED(NODE) ((NODE)->classtype.type_flags.has_call_overloaded)

/* Nonzero means that this _CLASSTYPE node overloads operator[].  */
#define TREE_HAS_ARRAY_REF_OVERLOADED(NODE) ((NODE)->classtype.type_flags.has_array_ref_overloaded)
...
#define TREE_ANY_ASSIGNS_THIS(NODE) ((NODE)->classtype.type_flags.any_assigns_this)
#define TREE_NONE_ASSIGN_THIS(NODE) ((NODE)->classtype.type_flags.none_assign_this)

/* Nonzero means that this _CLASSTYPE (or one of its ancestors) uses
   multiple inheritance.  If this is 0 for the root of a type
   hierarchy, then we can use more efficient search techniques.  */
#define TREE_USES_MULTIPLE_INHERITANCE(NODE) ((NODE)->classtype.type_flags.uses_multiple_inheritance)

#define CLASSTYPE_VIA_PUBLIC(NODE, N) ((NODE)->classtype.via_pub_or_virt[N]&1)
#define CLASSTYPE_VIA_VIRTUAL(NODE, N) ((NODE)->classtype.via_pub_or_virt[N]&2)
#define CLASSTYPE_VIAS(NODE) ((NODE)->classtype.via_pub_or_virt)
#define SET_CLASSTYPE_VIAS(NODE, N, PUB, VIRT) ((NODE)->classtype.via_pub_or_virt[N] = (PUB|(VIRT<<1)))

/* Mark bit for depth-first search.  */
#define CLASSTYPE_MARKED(NODE) ((NODE)->classtype.type_flags.marked)
#define CLASSTYPE_MARKED2(NODE) ((NODE)->classtype.type_flags.marked2)
#define CLASSTYPE_MARKED3(NODE) ((NODE)->classtype.type_flags.marked3)
#define CLASSTYPE_MARKED4(NODE) ((NODE)->classtype.type_flags.marked4)

struct tree_classtype
{
  struct tree_type typeinfo;
  ...
  int n_ancestors;
  union tree_node **by_bfs, **for_bfs;
  union tree_node **by_dfs, **for_dfs;
...
#define CLASSTYPE_CID(NODE) ((NODE)->classtype.cid)

#define CLASSTYPE_TAGS(NODE) ((NODE)->classtype.tags)
#define CLASSTYPE_OFFSET(NODE) ((NODE)->classtype.offset)
#define CLASSTYPE_VTABLE(NODE) ((NODE)->classtype.vtable)

#define CLASSTYPE_BASECLASSES(NODE) ((NODE)->classtype.types)

#define CLASSTYPE_N_BASECLASSES(NODE) ((NODE)->classtype.n_parents)
#define CLASSTYPE_BASECLASS(NODE, N) ((NODE)->classtype.types[N])

#define CLASSTYPE_N_SUPERCLASSES(NODE) ((NODE)->classtype.n_ancestors)
#define CLASSTYPE_SUPERCLASS_BY_BFS(NODE, N) ((NODE)->classtype.by_bfs[N])
#define CLASSTYPE_SUPERCLASS_BY_DFS(NODE, N) ((NODE)->classtype.by_dfs[N])
#define CLASSTYPE_SUPERCLASSES_BY_BFS(NODE) ((NODE)->classtype.by_bfs)
#define CLASSTYPE_SUPERCLASSES_FOR_BFS(NODE) ((NODE)->classtype.for_bfs)
#define CLASSTYPE_SUPERCLASSES_BY_DFS(NODE) ((NODE)->classtype.by_dfs)
#define CLASSTYPE_SUPERCLASSES_FOR_DFS(NODE) ((NODE)->classtype.for_dfs)

#define CLASSTYPE_NEXT_VARIANT(NODE) ((NODE)->classtype.next_class_variant)
#define CLASSTYPE_MAIN_VARIANT(NODE) ((NODE)->classtype.main_class_variant)
...
union tree_node
{
  ...
  struct tree_classtype classtype;
...
/* Given a type node TYPE, and PUBLICP and VIRTUALP, return a type
   for the same kind of data as TYPE describes.
   Variants point to the "class main variant" (which has neither PUBLIC nor VIRTUAL)
   via CLASSTYPE_MAIN_VARIANT, and it points to a chain of other variants
   so that duplicate variants are never made.
   Only main variants should ever appear as types of expressions.  */
