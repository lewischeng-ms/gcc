     ...
     tree type;
{
  ...
  if (value != 0 && (TREE_CODE (value) == VAR_DECL
	...
	case MEMBER_TYPE:
	  error ("invalid use of undefined member type");
	  return;
     ...
     tree t1, t2;
{
  ...
  switch (form1)
	...
	abort ();
      ...
      if (t1 == t2 || get_base_type (t1, t2))
	return t1;
      error ("commontype called with uncommon aggregate types (compiler error)");
      return t1;

    case MEMBER_TYPE:
      if (TYPE_MEMBER_OF (t1) == TYPE_MEMBER_OF (t2)
	  && TREE_CODE (TREE_TYPE (t1)) == TREE_CODE (TREE_TYPE (t2)))
	{
	  tree basetype = TYPE_MEMBER_OF (t1);
      ...
      error ("commontype called with uncommon member types (compiler error)");
      return t1;
     ...
     int strict;
{
  ...
  switch (TREE_CODE (t1))
    ...
    case MEMBER_TYPE:
      return (comptypes (TYPE_MEMBER_OF (t1), TYPE_MEMBER_OF (t2), strict)
	  ...
	  if (IS_AGGR_TYPE (t1) && IS_AGGR_TYPE (t2))
	    {
	      int rval;
  look_hard:
	      rval = t1 == t2 || get_base_type (t1, t2);

	      if (rval)
		return 1;
	      if (strict < 0)
		return (get_base_type (t2, t1) != 0);
	    }
	  return 0;
	}
      else
     ...
     tree ttl, ttr;
{
  ...
  if (TREE_CODE (ttr) == ARRAY_TYPE || TREE_CODE (ttr) == FUNCTION_TYPE || TREE_CODE (ttr) == POINTER_TYPE || TREE_CODE (ttr) == REFERENCE_TYPE)
    return comp_target_types (TREE_TYPE (ttl), TREE_TYPE (ttr));

  /* for C++ */
  else if (TREE_CODE (ttr) == MEMBER_TYPE)
    return comptypes (TYPE_MEMBER_OF (ttl), TYPE_MEMBER_OF (ttr), 0)
      && comp_target_types (TREE_TYPE (ttl), TREE_TYPE (ttr));
...
/* Given a structure or union value DATUM, construct and return
   ...
   x::A::L::ii refers to the ii member of the L part of
     ...
     tree types;
{
  ...
  if (TREE_CODE (types) == SCOPE_REF || TREE_USES_MULTIPLE_INHERITANCE (type))
    {
      ...
      while (TREE_CODE (types) == SCOPE_REF)
	{
	  ...
	  head->type = TREE_OPERAND (types, 1);
	  head->next = chain;
	  chain = head;
	  types = TREE_OPERAND (types, 0);
	}
      head = (struct type_chain *)alloca (sizeof (struct type_chain));
      head->type = TREE_OPERAND (types, 0);
      head->next = chain;
      chain = head;
      while (chain)
	{
	  if (is_aggr_typedef_or_else (chain->type))
	    {
	      tree basetype;

	      basetype = TREE_TYPE (TREE_TYPE (chain->type));
	      if (get_base_type (basetype, TYPE_MAIN_VARIANT (TREE_TYPE (TREE_TYPE (ref)))))
		ref = convert (TYPE_POINTER_TO (basetype), ref);
	      else
		{
		  error ("type `%s' is not derived from type `%s'",
			 TYPE_NAME_STRING (TREE_TYPE (TREE_TYPE (ref))),
			 IDENTIFIER_POINTER (chain->type));
		  return error_mark_node;
		}
	    }
	}
  ...
  if (is_aggr_typedef_or_else (types))
    {
	...
	case FIX_ROUND_EXPR:
	case FIX_CEIL_EXPR:
	  ref = convert (TYPE_POINTER_TO (TREE_TYPE (TREE_TYPE (types))),
     ...
     tree datum;
{
 ...
 more:
  ...
  if (TREE_CODE (TREE_TYPE (last_rval)) == POINTER_TYPE)
    return build_indirect_ref (last_rval, "dereferencing in build_x_arrow (compiler error)");

  if (TREE_CODE (TREE_TYPE (last_rval)) == MEMBER_TYPE)
    {
      if (TREE_CODE (last_rval) == MEMBER_REF
	  && TREE_STATIC (TREE_OPERAND (last_rval, 1)))
	{
	  last_rval = IDENTIFIER_GLOBAL_VALUE (DECL_STATIC_NAME (TREE_OPERAND (last_rval, 1)));
	  goto more;
	}
      error ("invalid member type in build_x_arrow (compiler error)");
      return error_mark_node;
    }
     ...
     tree datum, component;
{
  ...
  if (! comptypes (TYPE_MEMBER_OF (type), TREE_TYPE (datum), 0))
    {
      error ("member type `%s::' incompatible with object type `%s'",
	     TYPE_NAME_STRING (TYPE_MEMBER_OF (type)),
	     TYPE_NAME_STRING (TREE_TYPE (datum)));
      return error_mark_node;
    }

  return build (MEMBER_REF, TREE_TYPE (TREE_TYPE (component)), datum, component);
}
     ...
     int protect;
{
  ...
  field = lookup_field (basetype, component, ! VFIELD_NAME_P (component));
     ...
     tree function, params, decl;
{
  ...
  int is_method = ((TREE_CODE (function) == TREE_LIST
		    && IDENTIFIER_CLASS_VALUE (TREE_PURPOSE (function)) != 0)
		   || TREE_CODE (TREE_TYPE (function)) == MEMBER_TYPE);
  ...
  if (is_method)
  ...
  else if (TREE_CODE (function) == MEMBER_REF)
    {
      ...
      else
	{
      ...
      function = build_unary_op (ADDR_EXPR, TREE_OPERAND (function, 1), 0);

      if (TREE_VIRTUAL (ctype))
	{
	  e0 = build (NOP_EXPR, integer_type_node, function);
...
#endif
	  e3 = build_component_ref (decl, get_vfield_name (ctype), 1);
	  e3 = build_array_ref (build (NOP_EXPR, build_pointer_type (TREE_TYPE (function)), e3), e0);
      ...
      params = tree_cons (NULL_TREE, build_unary_op (ADDR_EXPR, decl, 0), params);
 ...
 do_x_function:
  {
    ...
    if (IS_AGGR_TYPE (type) && TREE_HAS_CALL_OVERLOADED (TYPE_MAIN_VARIANT (type)))
      return build_opfncall (CALL_EXPR, function, params);

    if (is_method)
      {
	tree ctypeptr = TYPE_POINTER_TO (TYPE_MEMBER_OF (TREE_TYPE (function)));
	if (decl == NULL_TREE)
	  {
	    error ("pointer to member function called, but not in class scope");
	    return error_mark_node;
     ...
     tree function, params;
{
  ...
  int is_method = TREE_CODE (TREE_TYPE (function)) == MEMBER_TYPE;
  ...
  if (TREE_CODE (fntype) != POINTER_TYPE)
    {
      error ("called object is not a pointer to function");
      return error_mark_node;
    }
...
/* Convert the actual parameter expressions in the list VALUES
   ...
   If any of these parameters lay into the stack, mark the head of
   the parameter list with TREE_PACKED.  */
     ...
     tree typelist, values, fndecl;
{
  ...
  if (fndecl)
    if (TREE_CODE (TREE_TYPE (fndecl)) == MEMBER_TYPE)
  ...
  for (valtail = values, typetail = typelist;
	...
	if (TREE_CODE (val) == TREE_LIST
	  ...
	  || TREE_CODE (TREE_TYPE (val)) == MEMBER_TYPE)
	...
	parm = build_tree_list (NULL_TREE, default_conversion (val));
      ...
      /* If we needed to create a temporary for this parameter, do so
	 in such a way that expand_decl will know to lay it directly
	 into the stack slot in which it will be passed.

	 If it is not moved by `convert_for_initialization' then it won't
	 mind being bitwise copied to the stack.  */
      if (type)
	if (TREE_NEEDS_CONSTRUCTOR (type)
	    && TREE_VALUE (parm) != val
	    && TREE_CODE (TREE_VALUE (parm)) == VAR_DECL
	    && TEMP_NAME_P (DECL_NAME (TREE_VALUE (parm))))
	  {
	    TREE_PACKED (TREE_VALUE (parm)) = 1;
	    packed = 1;
	  }
        /* wrong assumption.  */
      ...
      if (TREE_PURPOSE (typetail))
	{
	  while (TREE_VALUE (typetail) != void_type_node)
		...
		build_tree_list (0, convert_for_initialization (0, type, val, "default argument"));
		 ...
		 in such a way that expand_decl will know to lay it directly
		 into the stack slot in which it will be passed.  */
	      if (TREE_CODE (TREE_VALUE (parm)) == VAR_DECL
		  && TEMP_NAME_P (DECL_NAME (TREE_VALUE (parm)))
  ...
  if (packed && result != NULL_TREE)
    TREE_PACKED (result) = packed;
  ...
  switch (code)
      ...
      else if (code0 == POINTER_TYPE && code1 == POINTER_TYPE)
	{
	  ...
	  if (tt0 != tt1 && IS_AGGR_TYPE (tt0) && IS_AGGR_TYPE (tt1))
	    {
	      tree base = get_base_type (tt0, tt1);
	      int first_failed = (base == 0);
	      tree *argp;
	      if (first_failed)
		{
		  base = get_base_type (tt1, tt0);
		}
	      if (base == 0)
		warning ("comparison of distinct object pointer types");
	      else if (! integer_zerop (CLASSTYPE_OFFSET (base)))
		{
	  ...
	  else if (comp_target_types (dt0, dt1))
	    ;
     ...
     register tree ptrop, intop;
{
  ...
  else if (TREE_CODE (TREE_TYPE (result_type)) == MEMBER_TYPE)
    {
      if (pedantic)
     ...
     register tree op0, op1;
{
  ...
  if (pedantic)
	...
	warning ("pointer to a function used in subtraction");
      if (TREE_CODE (TREE_TYPE (dt0)) == MEMBER_TYPE)
  ...
  op1 = ((TREE_TYPE (dt0) == void_type_node
	  || TREE_CODE (TREE_TYPE (dt0)) == FUNCTION_TYPE
	  || TREE_CODE (TREE_TYPE (dt0)) == MEMBER_TYPE)
	 ? integer_one_node
     ...
     int noconvert;
{
  ...
  switch (code)
    ...
    case NEGATE_EXPR:
      if (IS_AGGR_TYPE_CODE (typecode))
	{
	  if (!noconvert)
	    arg = default_conversion (arg);
	  else
	    {
	      error ("type conversion for type `%s' no allowed",
		     TYPE_NAME_STRING (TREE_TYPE (arg)));
	      return error_mark_node;
	    }
	  typecode = TREE_CODE (TREE_TYPE (arg));
	  noconvert = 1;
	...
	{
	  tree name = TREE_PURPOSE (arg);
	  /* Look at methods with only this name.  */
	  if (TREE_CODE (TREE_VALUE (arg)) == FIELD_DECL)
	    {
	      arg = TREE_VALUE (arg);
		 ...
		 constructor, we can takes its address easily.  */
		  ...
		  || (DESTRUCTOR_NAME_P (DECL_NAME (TREE_TYPE (arg)))
		      && TREE_CHAIN (TREE_CHAIN (arg)) == NULL_TREE))
		{
		  if (TREE_CHAIN (arg))
		    arg = TREE_CHAIN (arg);
		  arg = build (MEMBER_REF, TREE_TYPE (TREE_TYPE (arg)), C_C_D, arg);

		  val = unary_complex_lvalue (ADDR_EXPR, arg);
		  if (val)
		    return val;
		}
	      warning ("address of (uninstantiated) function `%s::%s' ignored",
		       TYPE_NAME_STRING (DECL_CONTEXT (arg)),
	  ...
	  if (TREE_CHAIN (arg) == NULL_TREE)
	    {
	      /* ?? Unique overloaded function.  */
	      return build_unary_op (ADDR_EXPR, TREE_VALUE (arg), 0);
	    }
	  warning ("address of (uninstantiated) function `%s' ignored",
		   IDENTIFIER_POINTER (name));
	  return arg;
	}
     ...
     tree arg;
{
  ...
  if (code == ADDR_EXPR && TREE_CODE (TREE_TYPE (arg)) == MEMBER_TYPE)
    {
      /* The representation of something of type MEMBER_TYPE
	 is really the representation of a pointer to it.
	 Here give the representation its true type.  */
      tree t;
      tree offset;

      if (TREE_CODE (arg) == SCOPE_REF)
	abort ();
      ...
      if (TREE_CODE (arg) == MEMBER_REF)
	{
      ...
      if (TREE_CODE (TREE_TYPE (t)) == FUNCTION_DECL)
	{
	  if (TREE_VIRTUAL (t)
	    ...
	    {
	      offset = build_unary_op (ADDR_EXPR, TREE_TYPE (t), 0);
	    }
	  return offset;
	}
      else
	{
	  if (TREE_STATIC (t))
	    {
	      offset = build_unary_op (ADDR_EXPR, IDENTIFIER_GLOBAL_VALUE (DECL_STATIC_NAME (t)));
	    }
	  else
	    {
     ...
     tree ifexp, op0, op1;
{
  ...
  if (TREE_CODE (ifexp) == ERROR_MARK
      || TREE_CODE (TREE_TYPE (op0)) == ERROR_MARK
     ...
     enum tree_code modifycode;
{
  ...
  if (modifycode == INIT_EXPR)
	...
	lhstype = TREE_TYPE (lhstype);
      if (IS_AGGR_TYPE (lhstype) && TREE_HAS_ASSIGNMENT (TYPE_MAIN_VARIANT (lhstype)))
	{
  ...
  if (!pedantic)
	...
	if (TREE_CODE (TREE_TYPE (newrhs)) == ARRAY_TYPE
	    ...
	    || TREE_CODE (TREE_TYPE (newrhs)) == MEMBER_TYPE)
  ...
  /* SCOPE_REF is no longer used for this purpose.  */
  ...
  if (TREE_CODE (lhs) == MEMBER_REF)
	...
	if (TREE_CODE (member) == FIELD_DECL
	    && TREE_STATIC (member))
	  lhs = IDENTIFIER_GLOBAL_VALUE (DECL_STATIC_NAME (member));
	else
	  {
	    error ("invalid static class member (compiler error)");
	    return error_mark_node;
	  }
      }
    else
	...
	if (base == error_mark_node || member == error_mark_node)
	  return error_mark_node;
	lhs = build_indirect_ref (build (PLUS_EXPR, build_pointer_type (TREE_TYPE (lhs)),
					 build (NOP_EXPR, integer_type_node, base),
					 build (NOP_EXPR, integer_type_node, member)));
      }
  ...
  if (modifycode != INIT_EXPR
      ...
      && (lhstype == TREE_TYPE (rhs) || get_base_type (lhstype, TREE_TYPE (rhs))))
    {
      result = NULL_TREE;
     ...
     char *errtype;
{
  ...
  if (TREE_CODE (TREE_TYPE (rhs)) == ARRAY_TYPE
      ...
      || TREE_CODE (TREE_TYPE (rhs)) == MEMBER_TYPE)
    {
  ...
  /* Conversions among pointers */
	  ...
	  || TYPE_MAIN_VARIANT (ttr) == void_type_node)
	{
	  if (!((TYPE_MAIN_VARIANT (ttl) == void_type_node
		 && (!pedantic
			 ...
			 && TREE_CODE (ttr) != MEMBER_TYPE)))
		||
		(TYPE_MAIN_VARIANT (ttr) == void_type_node
		 && (!pedantic
		     || (ttr == void_type_node
			 && TREE_CODE (ttl) != FUNCTION_TYPE
			 && TREE_CODE (ttl) != MEMBER_TYPE)))))
      ...
      else if (comp_target_types (type, rhstype))
	{
      ...
      else if (TREE_CODE (TREE_TYPE (rhs)) == MEMBER_TYPE)
	{
     ...
     char *errtype;
{
  ...
  if (TREE_HAS_CONSTRUCTOR (type)
      || (IS_AGGR_TYPE (type) && IS_AGGR_TYPE (rhstype)))
    {
      if (type == TYPE_MAIN_VARIANT (rhstype)
	  || (IS_AGGR_TYPE (rhstype)
	      && get_base_type (type, TYPE_MAIN_VARIANT (rhstype))))
	{
	  if (TREE_GETS_INIT_REF (type))
	      ...
	      if (exp)
		{
		  ...
		  exp = convert_to_nonzero_pointer (TYPE_POINTER_TO (type), exp);
		  e0 = build_classfn_ref (exp, DECL_NAME (TYPE_NAME (type)),
					  build_tree_list (NULL_TREE, rhs), 1, 1);
		  e0 = build_indirect_ref (e0, "X(X&) constructor (compiler error)");
		  exp = build (COMPOUND_EXPR, TREE_TYPE (e1), e0, e1);
		}
	      else
     ...
     tree decl, init;
{
  ...
  if (IS_AGGR_TYPE (type) && TREE_NEEDS_CONSTRUCTOR (type))
    {
	      ...
	      && TYPE_MAIN_VARIANT (TREE_TYPE (init)) == type))
	{
	  ...
	  if (TREE_HAS_ASSIGN_REF (type))
	      ...
	      if (tmp != error_mark_node)
		{
		  ...
		  DECL_INITIAL (decl)
		    = build_compound_expr (tree_cons (NULL_TREE, init,
						      build_tree_list (NULL_TREE, tmp)));
		}
	      else
		{
     ...
     tree type, init, *tail;
{
  ...
  if (init && (TREE_TYPE (init) == type
	...
	warning ("ANSI C forbids initializing array from array expression");
      return maybe_convert_decl_to_const (init);
    }
     ...
     tree retval;
{
  ...
  if (!retval)
	...
	warning ("`return' with no value, in function returning non-void");
      cleanup_aggregates_for_return (0);
      expand_null_return ();
    }
	...
	{
	  do_aggr_init (DECL_RESULT (current_function_decl), retval, 0);
      ...
      if (any_aggregates_need_cleaning_p (0, save_from_destructor))
	{
	  ...
	  cleanup_aggregates_for_return (save_from_destructor);
	  emit_queue ();
	}
      else
	t = retval;
