     ...
     tree rec;
{
  ...
  for (i = 1; i <= CLASSTYPE_N_BASECLASSES (rec); i++)
    {
      int inc, desired_align;
      basetype = CLASSTYPE_BASECLASS (rec, i);
      CLASSTYPE_BASECLASS (rec, i) = build_classtype_variant (CLASSTYPE_BASECLASS (rec, i), convert_units (build_int (const_size), 1, BITS_PER_UNIT), 0);
  ...
  for (i = 1; i <= CLASSTYPE_N_BASECLASSES (rec); i++)
    {
      ...
      basetype = CLASSTYPE_BASECLASS (rec, i);
      CLASSTYPE_BASECLASS (rec, i) = build_classtype_variant (CLASSTYPE_BASECLASS (rec, i), convert_units (build_int (const_size), 1, BITS_PER_UNIT), 0);
      inc = MAX (union_align, TREE_INT_CST_LOW (TYPE_SIZE (basetype)) *
		 TYPE_SIZE_UNIT (basetype));
      const_size += inc;
    }
