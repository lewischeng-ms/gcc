	...
	case UNION_TYPE:
	  {
	    ...
	    tmp = next;
	    if (CLASSTYPE_BASECLASS (tmp, 1))
	      {
		records[i].baseclass = TYPE_SYMTAB_ADDRESS (CLASSTYPE_BASECLASS (tmp, 1));
		while (CLASSTYPE_BASECLASS (tmp, 1)
		       && TREE_VIRTUAL (CLASSTYPE_BASECLASS (tmp, 1)))
		  tmp = CLASSTYPE_BASECLASS (tmp, 1);
